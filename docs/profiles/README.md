...
| `python` | any Python project | [`dot_Brewfile.python`](../../dot_Brewfile.python) | [`run_once_after_install-project-python.sh.tmpl`](../../run_once_after_install-project-python.sh.tmpl) | [`python.md`](python.md) | **Language-type** profile — adds `ruff`, `pre-commit`, `httpie` on top of core (`uv`+`pyenv` already present). |
| `webdev` | any web/frontend project | [`dot_Brewfile.webdev`](../../dot_Brewfile.webdev) | [`run_once_after_install-project-webdev.sh.tmpl`](../../run_once_after_install-project-webdev.sh.tmpl) | [`webdev.md`](webdev.md) | **Language-type** profile — adds `bun`, `pnpm`, `vercel-cli` on top of core (`fnm`+Node already present). |
